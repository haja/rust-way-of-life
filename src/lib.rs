...
pub struct Game {
    cells: Vec<Vec<Cell>>, // TODO use single Vec, unfolding lines
    iteration_count: u64,
    wrapping: bool,
    height: u32,
    width: u32,
        ...
        next.cells = self
            ...
            .map(|(y, row)| {
                row.iter()
                    .enumerate()
                    .map(|(x, old_cell)| {
                        let neighbours = if self.wrapping {
                            get_neighbours_wrapping(self, x, y)
                        } else {
                            get_neighbours(self, x, y)
                        };
                        Cell {
...
fn get_two(game: &Game, x: usize, y: usize) -> Vec<&Cell> {
    [x as i32 - 1, x as i32 + 1]
        .iter()
        .map(|xi| get_if_positive(&game.cells[y], *xi))
        .flatten()
        .collect()
}
...
fn get_or_empty(vec: &[Vec<Cell>], x: usize, y: i32) -> Vec<&Cell> {
    if y >= 0 {
        vec.get(y as usize)
            .map(|row| get_three_of_row_or_empty(x, row))
            .unwrap_or_default()
    } else {
        Vec::new()
    }
...
fn get_neighbours_wrapping(game: &Game, x: usize, y: usize) -> Vec<&Cell> {
    let y_up = wrap(y as i32 - 1, game.height);
    let x_left = wrap(x as i32 - 1, game.width);
    let y_down = wrap(y as i32 + 1, game.height);
    let x_right = wrap(x as i32 + 1, game.width);

    vec![
        &game.cells[y_up][x_left],
        &game.cells[y_up][x],
        &game.cells[y_up][x_right],
        &game.cells[y][x_left],
        &game.cells[y][x_right],
        &game.cells[y_down][x_left],
        &game.cells[y_down][x],
        &game.cells[y_down][x_right],
    ]
}

fn wrap(idx: i32, len: u32) -> usize {
    let len = len as i32;
    if idx < 0 {
